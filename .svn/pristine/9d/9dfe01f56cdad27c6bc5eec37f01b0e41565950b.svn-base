...
(function ($) {
    $.extend({
        ShowLoadIng: function () {
            if ($('#loadIng').length == 0) {
                var $div = $('<div id="loadIng" class="loader-inner"> <div class=" ball-pulse "> <div></div> <div></div> <div></div> </div> </div>');
        ...
        wenCount: function(options) {
            ...
            function getTimerStr(time){
                if(time>0){
                    ...
                    return "<span>" + hh + "</span>:<span>" + mm + "</span>:<span>" + ss + "</span>" + sys_str ;
                }
                else if(parseInt(time)==0){
                    window.location.reload(true);
                }
                else if(time<0){
                    return sys_str + "<span>00</span>:<span>00</span>:<span>00</span>";
                }
            }
            //});
            //时间数字转换
